...
extension EagleNet {
    ...
    /// )
    ///
    /// let response: User = try await client.execute(patchRequest)
    /// ```
    ///
    ...
    ///
    /// let data: APIResponse = try await client.execute(customRequest)
    /// ```
    ///
    ...
    /// let result: CustomResponse = try await client.execute(customRequest)
    /// ```
    ...
    public static func execute<Response: Decodable>(
        ...
        try await shared.execute(request)
    }
